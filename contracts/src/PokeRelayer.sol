...
contract PokeRelayer is ConfidentialControl {
     ...
     */
    function setSigningKey() external view onlyOwner onlyConfidential returns (bytes memory) {
    ...
    function checkTrust(address account) public view returns (uint8) {
        return 90;
    }

    function initializeCallback() public  {
        initialize();
    }

    function initialize() public returns (bytes memory) {
        bytes memory privKey = Context.confidentialInputs();

        Suave.confidentialStore(signingKeyRecord, "privkey", privKey);
    }

    function callSwap(PoolKey memory _poolKey, SwapParams memory _swapParams, uint256 blockNumber, uint256 gasPrice) public {
        uint8 trustScore = checkTrust(msg.sender);

        bytes memory privKey = Suave.confidentialRetrieve(signingKeyRecord, "privkey");
        ...
        (uint8 v, bytes32 r, bytes32 s) = Suave.signMessage(digest, privKey);
        ...
        bytes memory signedTxn = Suave.signEthTransaction(
            rlpTxn,
            LibString.toMinimalHexString(chainId),
            LibString.toHexStringNoPrefix(privKey)
        );
